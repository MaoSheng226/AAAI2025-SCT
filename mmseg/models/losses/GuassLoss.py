...
def Guass_L1_loss(pred,
    ...
    GT_trance = generate_distance(target).to(torch.int64)
    ...
    GTtrance_one = GT_trance
    if (pred3.shape != target.shape):
        GTtrance_one = _expand_onehot_labels_dice(distance,  GT_trance)
    ...
    sumloss = l1_loss(pred3, GT3one_hot_target) + l1_loss(pred2, GT2one_hot_target) + l1_loss(pred1, GT1one_hot_target)+ l1_loss(distance, GTtrance_one)
    loss = sumloss
...
class GussL1_Loss(nn.Module):
    ...
    def __init__(self,
                 ...
                 kernel=5,
                 std=2,
                 loss_name='loss_guassl1'):
        """Compute guass l1 loss.
    ...
    def forward(self,
        ...
        loss = self.loss_weight * Guass_L1_loss(
            pred,
            target,
            weight,
            reduction= reduction,
